...
class DataValidation:
    ...
    def is_train_test_file_exist(self):
        try:
            logging.info("Checking if training and test file path available")
    ...
    def validate_dataset_schema(self):
        try:
            ...
            #write code to validate training and testing dataset using schema file
            #1.  Num of column
            #2. domain values (ocean proximity)
            # acceptable values
            #3. column names
    ...
    def save_data_drift_report_page(self):
        try:
            dashnoard = Dashboard(tabs=[DataDriftTab()])
            train_df, test_df = self.get_train_test_df()
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            data_validation_artifact = DataValidationArtifact(
                schema_file_path = self.data_validation_config,
                report_file_path = self.data_validation_config.report_file_path,
                report_page_file_path = self.data_validation_config.report_page_file_path,
                is_validated = "Data validation performed successufully"
            )
